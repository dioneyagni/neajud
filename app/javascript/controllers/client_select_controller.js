...
export default class extends Controller {
  ...
  _doSearch() {
    ...
    fetch(`${this.searchUrlValue}?q=${encodeURIComponent(q)}`, { signal: this.abortController.signal })
      .then(r => r.json())
      .then(items => {
        this.resultsTarget.innerHTML = items.map(item =>
          `<div class="combobox-option" data-id="${item.id}" data-name="${item.name}" data-action="click->client-select#select">
            <strong>${this._highlight(item.name, q)}</strong>
            ${item.responsible ? `<span class="combobox-option-detail">${this._highlight(item.responsible, q)}</span>` : ""}
          </div>`
        ).join("") + `
          <div class="combobox-option combobox-option-new" data-action="click->client-select#openNew">
            + Register new
          </div>`
